    ...
    <div class="all-courses">
      <div class="page-header">
        <div>
          ...
          <span class="page-sub">{{ activeTab === 'reviews' ? reviews.length : discussions.length }} 条{{ activeTab === 'reviews' ? '评价' : '讨论' }}</span>
        </div>
      </div>

      <!-- 分区切换 -->
      <div class="seg admin-seg">
      ...
      <!-- 讨论列表 -->
      <div v-show="activeTab === 'discussions'" class="review-list">
        <div v-for="d in discussions" :key="d.id" class="glass-card">
          <div class="review-head">
            <div class="review-avatar">
              {{ d.isAnonymous ? '?' : (d.username ? d.username.charAt(0).toUpperCase() : '?') }}
            </div>
            <div class="review-info">
              <div class="review-header-row">
              ...
              <div class="review-meta">
                ...
                <span class="discuss-badge">讨论</span>
              </div>
            </div>
          </div>
          ...
          <div class="review-stats">
            <span>{{ d.likeCount || 0 }} 赞</span>
            <span class="stat-dot"></span>
            <span>{{ d.dislikeCount || 0 }} 踩</span>
            <span class="stat-dot"></span>
            <span>{{ (discussionRepliesMap[d.id] || []).length }} 回复</span>
            <button class="btn-delete-review" @click="handleDeleteDiscussion(d.id)">
          ...
          <button v-if="!showDiscussionReplies[d.id]" class="toggle-btn" @click="loadDiscussionReplies(d.id)">
            <svg viewBox="0 0 20 20" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round">
              <path d="M4 7l6 6 6-6"/>
            </svg>
            查看回复
          </button>
          <button v-else class="toggle-btn" @click="showDiscussionReplies[d.id] = false">
            <svg viewBox="0 0 20 20" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round">
              <path d="M4 13l6-6 6 6"/>
            </svg>
            收起回复
          ...
          <div v-if="showDiscussionReplies[d.id] && discussionRepliesMap[d.id]" class="replies-section">
            <div v-for="reply in discussionRepliesMap[d.id]" :key="reply.id" class="reply-item">
              ...
              <div class="reply-body">
                ...
                <div class="reply-footer">
                  <span class="reply-likes">{{ reply.likeCount || 0 }} 赞 · {{ reply.dislikeCount || 0 }} 踩</span>
                  <button class="btn-delete-reply" @click="handleDeleteDiscussionReply(d.id, reply.id)">
                    <svg viewBox="0 0 20 20" width="12" height="12" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                      <path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/>
                    </svg>
                    删除
      ...
      <div v-if="activeTab === 'reviews' && reviews.length === 0 && !loading" class="empty-state">
      ...
      <div v-if="activeTab === 'discussions' && discussions.length === 0 && !loading" class="empty-state">
        <div class="empty-icon-wrap">
          <svg viewBox="0 0 48 48" fill="none" stroke="currentColor" width="36" height="36">
...
const loadDiscussionReplies = (discussionId) => {
  get(`/api/course/community/replies?reviewId=${discussionId}`, (data) => {
    ...
    showDiscussionReplies.value[discussionId] = true
  })
}
...
.review-date { font-size: 12px; color: #000000; }
...
.reply-time { font-size: 11px; color: #000000; margin-left: auto; }
...
.btn-delete-review {
  ...
  border: none; background: rgba(255,59,48,0.08);
  color: #ff3b30; font-size: 12px; font-weight: 600;
  font-family: inherit; cursor: pointer;
  transition: all 0.15s;
}
